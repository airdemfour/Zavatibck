...
if(1){
	...
	Class ZORM{
		...
		*/
		static function create($table, $description=array()){
		...
		*/
		static function truncate($tables= array()){
			if(is_array($table)){
		...
		*/
		static function find($col, $val){
			...
			return count($records) == 0 ? array() : $records;
		}
	...
	*/
	class ORMCxn {
		...
		public function __construct($adapter=false, $db=false, $user=false, $pass=false, $host=false){
			...
			$this->pass = $pass;
			$this->db = $db;
			$this->pdo = self::mysql($db, $user, $pass, $host);
		}
		...
		public function mssql($db, $user, $pass, $host){
			return new PDO("sqlsrv:Server=localhost,1433;Database=nipost_estamp", $user , $pass);
			//return new PDO ("odbc:mssql", $this->user, $this->pass);
		}
